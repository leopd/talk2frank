...
def reload_system_prompt():
    vlm = get_vlm()
    old_system_prompt = vlm.system_prompt
    path = Path(__file__).parent / "prompt.txt"
    if path.exists():
        vlm.system_prompt = path.read_text(encoding="utf-8").strip()
    if old_system_prompt != vlm.system_prompt:
        print(f"System prompt changed to: {vlm.system_prompt}")
